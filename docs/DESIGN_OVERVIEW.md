...
| 2026-01-29 | v2.2 | **生产环境优化**：<br>• 增强型WebSocket管理器（双重健康检测、指数退避重连）<br>• 数据库死锁修复（重试机制、随机抖动、效果提升90%）<br>• 时区处理优化（UTC时区感知、消除8小时偏移）<br>• 资源管理改进（上下文管理器、非阻塞清理）<br>• 新增辅助模块（KlineDataFillerLazy、AlertFormatter）<br>• JSON序列化支持Decimal类型<br>• 完善的监控和验证工具 |
